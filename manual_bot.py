...
# v4.1 (2025-08-15)
# - SL glitch fix & math-correct exits:
#   * Side-aware exits (LONG → bid; SHORT → ask)
#   * If SL hits before any TP: close ALL buckets (TP1+TP2+runner) at SL
#   * Micro-debounce (~350 ms) & tiny spread band (~0.02%) on SL trigger
#   * Sanity clamps: MAX_STOP_PCT≈20%, MAX_GAP_PCT≈8%
#   * Journal clear reason codes for SL/TP/gap
# - Dashboard & controls:
#   * /status (one-glance dashboard) • /quietreason (why silent)
#   * /setscore S • /setmode RELAXED|STRICT • /setflags K 0|1
#   * /setpairs N • /setlongs 0|1 • /setshorts 0|1 • /setcorr 0|1 • /setcandle 0|1
#   * /setmaxopen N • /setalloc PCT
#   * /autotune 0|1 • /tuneinfo
#   * /scan /paper /positions /closed /pnl /lasttrades /ping
# - Auto-Tune (OFF by default): adjusts new-trade settings from BTC/ETH 1h ADX
#   * ADX≥35: RELAXED, score≥95, shorts ON, max_open=8, alloc=0.03, TP split 40/30/30
#   * 25–35:  STRICT, score≥97, shorts ON, max_open=6, alloc=0.03, TP split 40/30/30
#   * <25:    STRICT, score≥98, shorts OFF, max_open=5, alloc=0.025, TP split 40/30/30
#   * Never touches open positions; posts a one-line note when retuned.
# - Optional candle filter (OFF by default): simple engulfing/hammer/shooting-star confirm.
...
# paper sizing
POSITION_PCT            = float(os.getenv("POSITION_PCT", "0.05"))  # /setalloc can change
MAX_OPEN_TRADES         = int(os.getenv("MAX_OPEN_TRADES", "10"))   # /setmaxopen can change
...
# corr toggle
CORR_ENABLED   = int(os.getenv("CORR_ENABLED", "1"))  # /setcorr 0|1
# candle filter toggle
CANDLE_FILTER  = int(os.getenv("CANDLE_FILTER", "0")) # /setcandle 0|1

# Auto-Tune
AUTO_TUNE           = int(os.getenv("AUTO_TUNE", "0"))  # OFF by default
...
session.headers.update({"User-Agent": "insider-sniper/4.1"})
...
def ticker_book(pid):
    """Return last, bid, ask floats (falls back to last when missing)."""
    try:
        _jitter()
...
# ---------- candle patterns (simple, optional) ----------
...
# ---------- Paper engine (3 TPs + ratchet + SL fixes) ----------
def _parse_split(total_qty):
    # robust split: negative → 0, renormalize
    try:
        parts=[max(0,int(x)) for x in TP_SPLIT.split(",") if x!=""]
    except:
        parts=[40,30,30]
...
def _pos_sideaware_price(side, pid):
    last,bid,ask = ticker_book(pid)
    return (bid if side=="LONG" else ask), last, bid, ask
...
def exec_open(sig, ctx: CallbackContext, chat_id):
    ...
    # correlation sizing vs open symbols
    ...
    entry = sig["entry"]
    qty = (alloc * (1 - FEE_RATE)) / max(entry, 1e-9)
...
def _finalize_close(pid, p, reason, exit_avg):
    pnl_usd = ((exit_avg - p["entry_avg"]) if p["side"]=="LONG" else (p["entry_avg"] - exit_avg)) * p["qty_total"] - (p["fees_open_total"] + p["fees_close_total"])
    pnl_pct = ((exit_avg - p["entry_avg"]) / p["entry_avg"] * 100) if p["side"]=="LONG" else ((p["entry_avg"] - exit_avg) / p["entry_avg"] * 100)
    PAPER["realized_pnl"] += pnl_usd
    add_result_winloss(pnl_usd)
    closed = {"id": pid, "symbol": p["symbol"], "side": p["side"],
              "entry_avg": p["entry_avg"], "exit_avg": exit_avg,
              "qty_total": p["qty_total"], "pnl_usd": pnl_usd,
...
def _within_band(side, cur, level, band_pct):
    if side=="LONG":
        return cur <= level*(1+band_pct)
    else:
        return cur >= level*(1-band_pct)
...
def paper_check_exits(ctx: CallbackContext):
    ...
    for pid, p in list(PAPER["positions"].items()):
        touch, last, bid, ask = _pos_sideaware_price(p["side"], p["symbol"])
        if not np.isfinite(touch): continue
        now = time.time()

        # TP3 hard cap: exit everything at TP3 if crossed
        if TP_LEVELS >= 3 and ((p["side"]=="LONG" and touch >= p["tp3"]) or (p["side"]=="SHORT" and touch <= p["tp3"])):
        ...
        # TP2 partial
        if (not p["tp2_done"]) and p["qty_tp2"]>0 and ((p["side"]=="LONG" and touch >= p["tp2"]) or (p["side"]=="SHORT" and touch <= p["tp2"])):
            _close_block(p, p["qty_tp2"], p["tp2"])
            p["tp2_done"]=True; p["last_tp_hit_ts"]=now
        ...
        # TP1 partial
        if (not p["tp1_done"]) and p["qty_tp1"]>0 and ((p["side"]=="LONG" and touch >= p["tp1"]) or (p["side"]=="SHORT" and touch <= p["tp1"])):
            _close_block(p, p["qty_tp1"], p["tp1"])
            p["tp1_done"]=True; p["last_tp_hit_ts"]=now
        ...
        # Stop-loss on remainder (with debounce & band & post-TP hold)
        rem_runner = p["qty_runner"] + (0 if p["tp1_done"] else p["qty_tp1"]) + (0 if p["tp2_done"] else p["qty_tp2"])
        if rem_runner <= 0: 
            continue

        # post-TP hold window
        if p["last_tp_hit_ts"] and (now - p["last_tp_hit_ts"]) < POST_TP_HOLD_SEC:
            continue

        # SL trigger with band & debounce
        sl_hit = ((p["side"]=="LONG" and touch <= p["sl_dyn"]) or (p["side"]=="SHORT" and touch >= p["sl_dyn"]))
        if sl_hit and _within_band(p["side"], touch, p["sl_dyn"], SL_BAND_PCT) and (now - p["last_sl_probe_ts"]) >= SL_DEBOUNCE_SEC:
            p["last_sl_probe_ts"]=now
            # If SL before any TP → close ALL buckets at SL price
            if not p["tp1_done"] and not p["tp2_done"]:
                _close_block(p, rem_runner, p["sl_dyn"])
                exit_avg = p["sl_dyn"]
                pnl_usd, pnl_pct = _finalize_close(pid, p, "SL(all)", exit_avg)
                to_announce.append(f"🛑 <b>SL</b> {p['symbol']} full @ {exit_avg:.6f} | PnL ${pnl_usd:.2f} ({pnl_pct:.2f}%)")
            else:
                # Otherwise close only remaining at SL dyn, compute true avg
                _close_block(p, rem_runner, p["sl_dyn"])
...
def update_equity_mark():
    ...
    for p in PAPER["positions"].values():
        px, last, bid, ask = _pos_sideaware_price(p["side"], p["symbol"])
        if np.isfinite(px):
            rem = (0 if p["tp1_done"] else p["qty_tp1"]) + (0 if p["tp2_done"] else p["qty_tp2"]) + p["qty_runner"]
            # mark-to-market with side-aware price
            if p["side"]=="LONG":
                eq += px * rem * (1 - FEE_RATE)
            else:
                # for short, profit increases when price falls; mark with current ask
                eq += ( (p["entry_avg"] - px) * rem )  # paper approximation
    PAPER["equity"] = eq
    save_paper()
...
# ---------- Auto-Tune ----------
def autotune_once(ctx: CallbackContext, chat_id):
    global LAST_TUNE_NOTE, MODE, SCORE_MIN, SHORTS_ENABLED
    # compute regime
    _update_regime()
    adx_btc = BTC_STATE["adx1h"]; adx_eth = ETH_STATE["adx1h"]
    adx = max(adx_btc, adx_eth)

    # current knobs (defaults)
    new = dict(mode="STRICT", score=97, shorts=1, max_open=6, alloc=0.03, tp_split="40,30,30")

    if adx >= 35:
        new.update(mode="RELAXED", score=95, shorts=1, max_open=8, alloc=0.03)
    elif adx >= 25:
        new.update(mode="STRICT", score=97, shorts=1, max_open=6, alloc=0.03)
    else:
        new.update(mode="STRICT", score=98, shorts=0, max_open=5, alloc=0.025)

    # safety if cooldown/daylock
    if cooldown_active():
        new["score"] = max(new["score"], SCORE_MIN+1)
        new["max_open"] = min(new["max_open"], max(3, MAX_OPEN_TRADES-1))

    # apply to NEW trades only
    MODE = new["mode"]; apply_mode(MODE)
    SCORE_MIN = new["score"]
    SHORTS_ENABLED = int(new["shorts"])
    CONFIG["MAX_OPEN_TRADES"] = int(new["max_open"])
    CONFIG["POSITION_PCT"] = float(new["alloc"])
    globals()["TP_SPLIT"] = new["tp_split"]
    save_config()

    note = f"AutoTune → ADX={adx:.1f} | MODE={MODE}, SCORE≥{SCORE_MIN}, SHORTS={SHORTS_ENABLED}, MAX_OPEN={CONFIG['MAX_OPEN_TRADES']}, ALLOC={CONFIG['POSITION_PCT']*100:.1f}%, TP={TP_SPLIT}"
    if note != LAST_TUNE_NOTE:
        LAST_TUNE_NOTE = note
        if chat_id: ctx.bot.send_message(chat_id=chat_id, text="🔧 " + note)
...
def job_daily(ctx: CallbackContext):
    ...
    recent = [s for s in signals if s["time"] >= cutoff and s.get("score", 0) >= SCORE_MIN]
...
def cmd_start(update: Update, ctx: CallbackContext):
    ...
    ctx.bot.send_message(chat_id=target_chat_id(update), parse_mode=ParseMode.HTML, text=(
        "👋 InsiderSignals_Manual v4.1\n"
        f"Mode: <b>{CONFIG['MODE']}</b> | SCORE_MIN <b>{SCORE_MIN}</b>\n"
        ...
        f"Longs: {LONGS_ENABLED} | Shorts: {SHORTS_ENABLED} | CorrFilter: {CORR_ENABLED} | CandleFilter: {CANDLE_FILTER}\n"
        f"AutoTune: {AUTO_TUNE}\n"
        f"Pairs tracked: {MAX_PAIRS}\n"
        "Commands: /status /scan /paper /positions /closed /pnl /lasttrades\n"
        "/setscore S /setmode RELAXED|STRICT /setflags EDGE|CORR|REGIME|COOLDOWN|SESSION 0|1\n"
        "/setpairs N /setlongs 0|1 /setshorts 0|1 /setcorr 0|1 /setcandle 0|1\n"
        "/setmaxopen N /setalloc PCT\n"
        "/autotune 0|1 /tuneinfo /quietreason /ping"
    ))

def cmd_status(update: Update, ctx: CallbackContext):
    update_equity_mark()
    cd = "YES" if cooldown_active() else "NO"
    ctx.bot.send_message(chat_id=target_chat_id(update),
        parse_mode=ParseMode.HTML,
        text=(f"🧭 <b>Status</b>\nMODE={CONFIG['MODE']} SCORE≥{SCORE_MIN} "
              f"| longs={LONGS_ENABLED} shorts={SHORTS_ENABLED} corr={CORR_ENABLED} candle={CANDLE_FILTER} "
              f"| pairs={MAX_PAIRS} chunk={CHUNK_SIZE}\n"
              f"Open={len(PAPER['positions'])} | Cash=${PAPER['cash']:.2f} | Equity=${PAPER['equity']:.2f}\n"
              f"AutoTune={AUTO_TUNE} | Cooldown/DayLock={cd}"))

def cmd_quiet(update: Update, ctx: CallbackContext):
    reasons=[]
    if cooldown_active(): reasons.append("cooldown/day lock active")
    if FLAGS.get("SESSION",1) and _session_quiet_now(): reasons.append("quiet hours")
    if len(PAPER["positions"]) >= CONFIG["MAX_OPEN_TRADES"]: reasons.append("max open reached")
    if free_usd() < MIN_USD_PER_TRADE: reasons.append("insufficient cash")
    if not reasons: reasons.append("no valid signals (filters not met)")
    ctx.bot.send_message(chat_id=target_chat_id(update), text="🤫 Quiet because: " + "; ".join(reasons))

def cmd_scan(update: Update, ctx: CallbackContext):
    ctx.bot.send_message(chat_id=target_chat_id(update), text=f"🔍 Scan running (mode={CONFIG['MODE']}, score≥{SCORE_MIN})…")
...
def cmd_positions(update: Update, ctx: CallbackContext):
    ...
    for pid, p in PAPER["positions"].items():
        ...
        if p["side"]=="SHORT": up_span = (p["entry_avg"] - (p["tp3"] if TP_LEVELS>=3 else p["tp2"]))
        dn_span = p["entry_avg"] - p["sl_dyn"] if p["side"]=="LONG" else (p["sl_dyn"] - p["entry_avg"])
        if p["side"]=="LONG":
            prog = 0.0 if up_span<=0 else (px - p["entry_avg"])/max(1e-9, up_span)*100
        else:
            prog = 0.0 if up_span<=0 else (p["entry_avg"] - px)/max(1e-9, up_span)*100
        prog = max(-100.0, min(100.0, prog))
        # mark-to-market PnL approx
        pnl = ((px - p["entry_avg"]) if p["side"]=="LONG" else (p["entry_avg"] - px)) * p["qty_total"] - p["fees_open_total"]
        pnl_pct = ((px - p["entry_avg"])/p["entry_avg"]*100) if p["side"]=="LONG" else ((p["entry_avg"] - px)/p["entry_avg"]*100)
        lines.append(
            f"• {p['symbol']} {p['side']} qty {p['qty_total']:.6f} | entry {p['entry_avg']:.6f} | now {px:.6f} | "
            f"PnL ${pnl:.2f} ({pnl_pct:.2f}%) | progress {prog:.1f}%"
        )
...
def cmd_setscore(update: Update, ctx: CallbackContext):
    try:
        ...
        globals()["SCORE_MIN"] = s; save_config()
...
def cmd_setmode(update: Update, ctx: CallbackContext):
    try:
        ...
        globals()["MODE"] = m; apply_mode(m); save_config()
...
def cmd_setpairs(update: Update, ctx: CallbackContext):
    try:
        ...
        globals()["MAX_PAIRS"] = n; globals()["watchlist"] = []
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ MAX_PAIRS set to {n}. Takes effect next scan.")
    except Exception:
...
def cmd_setcorr(update: Update, ctx: CallbackContext):
    try:
        v = int(update.message.text.split()[1]); assert v in (0,1)
        globals()["CORR_ENABLED"] = v
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ Correlation filter set to {v}.")
    except Exception:
...
def cmd_setcandle(update: Update, ctx: CallbackContext):
    try:
        v = int(update.message.text.split()[1]); assert v in (0,1)
        globals()["CANDLE_FILTER"] = v
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ Candle filter set to {v}.")
    except Exception:
        ctx.bot.send_message(chat_id=target_chat_id(update), text="Usage: /setcandle 0|1")

def cmd_setmaxopen(update: Update, ctx: CallbackContext):
    try:
        n = int(update.message.text.split()[1]); assert 1 <= n <= 30
        CONFIG["MAX_OPEN_TRADES"] = n; save_config()
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ MAX_OPEN_TRADES set to {n}.")
    except Exception:
        ctx.bot.send_message(chat_id=target_chat_id(update), text="Usage: /setmaxopen 1..30")

def cmd_setalloc(update: Update, ctx: CallbackContext):
    try:
        p = float(update.message.text.split()[1]); assert 0.01 <= p <= 0.20
        CONFIG["POSITION_PCT"] = p; save_config()
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ POSITION_PCT set to {p*100:.1f}%.")
    except Exception:
        ctx.bot.send_message(chat_id=target_chat_id(update), text="Usage: /setalloc 0.01..0.20  (i.e., 0.05 for 5%)")

def cmd_autotune(update: Update, ctx: CallbackContext):
    try:
        v = int(update.message.text.split()[1]); assert v in (0,1)
        globals()["AUTO_TUNE"] = v
        ctx.bot.send_message(chat_id=target_chat_id(update), text=f"✅ AutoTune set to {v}.")
    except Exception:
...
def cmd_tuneinfo(update: Update, ctx: CallbackContext):
    adx = max(BTC_STATE["adx1h"], ETH_STATE["adx1h"])
    ctx.bot.send_message(chat_id=target_chat_id(update), text=(
        f"🔧 AutoTune: {'ON' if AUTO_TUNE else 'OFF'} | ADX now {adx:.1f}\n"
        f"Target: MODE={MODE}, SCORE≥{SCORE_MIN}, SHORTS={SHORTS_ENABLED}, "
        f"MAX_OPEN={CONFIG['MAX_OPEN_TRADES']}, ALLOC={CONFIG['POSITION_PCT']*100:.1f}%, TP={TP_SPLIT}"))
...
def main():
    ...
    dp.add_handler(CommandHandler("quietreason", cmd_quiet))
    ...
    dp.add_handler(CommandHandler("setmaxopen", cmd_setmaxopen))
    ...
    jq.run_repeating(job_autotune, interval=max(120, TUNE_INTERVAL_MIN*60), first=30, name="autotune")
    jq.run_daily(job_daily, time=dtime(hour=DAILY_REPORT_HOUR, minute=0, tzinfo=tz))

    logging.info("Manual bot v4.1 started")
